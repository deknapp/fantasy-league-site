...
//
//   node scripts/snapshot-league.mjs "<fantasy.espn.com league URL>"
//
// Private leagues: set ESPN_S2 and SWID environment variables.
...
if (!parsed) {
  console.error('Usage: node scripts/snapshot-league.mjs "<fantasy.espn.com league URL>"');
  process.exit(2);
}

const { sport, leagueId, year } = parsed;
const url = leagueApiUrl(sport, year, leagueId);
const headers = { Accept: "application/json", "User-Agent": "Mozilla/5.0 (compatible; fantasy-league-site)" };
if (process.env.ESPN_S2 && process.env.SWID) headers.Cookie = `espn_s2=${process.env.ESPN_S2}; SWID=${process.env.SWID}`;
